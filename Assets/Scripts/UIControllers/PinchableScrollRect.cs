   ...
   public float minZoom = 1f;
    ...
    private void Update()
    {
        ...
        if (Mathf.Abs(scrollWheelInput) > float.Epsilon)
        {
            ...
            _currentZoom = Mathf.Clamp(_currentZoom, minZoom, _maxZoom);
    ...
    void OnPinchStart()
    {
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(content, _startPinchScreenPosition, null, out _startPinchScreenPosition);
        Vector2 pivotPosition = new Vector3(content.pivot.x * content.rect.size.x, content.pivot.y * content.rect.size.y);
        Vector2 posFromBottomLeft = pivotPosition + _startPinchScreenPosition;
        SetPivot(content, new Vector2(posFromBottomLeft.x / content.rect.width, posFromBottomLeft.y / content.rect.height));
    ...
    static void SetPivotWithoutPosition(RectTransform rectTransform, Vector2 pivot)
    {
        if (rectTransform == null) return;
    ...
    static void SetPosition(RectTransform rectTransform, Vector2 pivot)
    {
        if (rectTransform == null) return;
