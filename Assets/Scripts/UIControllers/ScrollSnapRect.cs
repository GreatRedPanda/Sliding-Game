...
{
    public event System.Action<int> OnPageSwitched;
    ...
    public Transform PageIconPrefab;
    ...
    void Update()
    {
        //Debug.Log("TESTING TIME" + Time.deltaTime + "   " + Time.fixedDeltaTime);
        ...
        if (_lerp)
        {
            ...
            float decelerate = Mathf.Min(decelerationRate *0.02f, 1f);
    ...
    private void SetPagePositions()
    {
        ...
        if (_horizontal)
        {
            ...
            anchoredMaxX = _scrollRectRect.rect.width / (_scrollRectRect.rect.width * _pageCount + _fastSwipeThresholdMaxLimit);
        }
        else
        {
        ...
        {
            GridLayoutGroup gridLayoutGroup = pageSelectionIcons.GetComponent<GridLayoutGroup>();
            RectTransform rectTransform = pageSelectionIcons.GetComponent<RectTransform>();
            ...
            float cellSizeX = (rectTransform.rect.width - paddingX) / _pageCount;
        ...
        Vector2 size = _container.parent.GetComponent<RectTransform>().rect.size;
        float w = 0;
        ...
        for (int i = 0; i < _pageCount; i++)
        {
            ...
            if (_horizontal)
            {
                //childPosition = new Vector2(i * width, 0);
            ...
            child.anchorMax = new Vector2(anchoredMaxX, 1);
            child.offsetMin = new Vector2(0, 0);
            child.offsetMax = new Vector2(0, 0);
            //newPage.localPosition = new Vector2(i * size.x, 0);
            w = child.rect.width;

            child.anchoredPosition = new Vector2(i * width + offsetX, 0);
            childPosition = new Vector2(i * width - containerWidth / 2 + offsetX, 0f);
            ...
            if (pageSelectionIcons != null)
            {
                Transform pageIcon = Instantiate(PageIconPrefab);


                pageIcon.SetParent(pageSelectionIcons);
                pageIcon.localScale = Vector3.one;
            }
        }
    ...
    List<RectTransform> _pages = new List<RectTransform>();
    //------------------------------------------------------------------------
    ...
    private void NextScreen()
    {
        ...
        OnPageSwitched?.Invoke(_currentPage);
    }
    ...
    void SwitchPage(int currentPage)
    {


        for (int i = 0; i < _pages.Count; i++)
        {
            if (i == currentPage || i - 1 == currentPage || i + 1 == currentPage)
            {
